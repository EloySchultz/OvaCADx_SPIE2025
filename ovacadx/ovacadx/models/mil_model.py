# mostly copy paste from MONAI with some adaptations: 
# https://docs.monai.io/en/stable/_modules/monai/networks/nets/milmodel.html#MILModel
...
from monai.networks.nets.resnet import ResNetBlock as ResNetBlockMONAI,  ResNetBottleneck as ResNetBottleneckMONAI
from monai.utils.module import optional_import

from ovacadx.ssl.models.resnet import ResNetBlock, ResNetBottleneck
...
class MILModel(nn.Module):
    ...
    Args:
        ...
        backbone_num_features: Number of output features of the backbone
    ...
    def __init__(
        ...
        if self.mil_mode in ["mean", "max"]:
            ...
            if type(net.layer1[-1]) == ResNetBlock or type(net.layer1[-1]) == ResNetBlockMONAI:
                final_layer = "bn2"
            elif type(net.layer1[-1]) == ResNetBottleneck or type(net.layer1[-1]) == ResNetBottleneckMONAI:
                final_layer = "bn3"
            else:
                raise ValueError("Unsupported ResNet block type")
            
            layer1_out_channels = getattr(net.layer1[-1], final_layer).num_features
            layer2_out_channels = getattr(net.layer2[-1], final_layer).num_features
            layer3_out_channels = getattr(net.layer3[-1], final_layer).num_features
            layer4_out_channels = getattr(net.layer4[-1], final_layer).num_features

            transformer_list = nn.ModuleList(
                [
                    nn.TransformerEncoder(
                        nn.TransformerEncoderLayer(d_model=layer1_out_channels, nhead=8, dropout=trans_dropout), num_layers=trans_blocks
                    ),
                    nn.Sequential(
                        nn.Linear(layer1_out_channels + layer2_out_channels, layer1_out_channels),
                        nn.TransformerEncoder(
                            nn.TransformerEncoderLayer(d_model=layer1_out_channels, nhead=8, dropout=trans_dropout),
                            num_layers=trans_blocks,
                        ),
                    ),
                    nn.Sequential(
                        nn.Linear(layer1_out_channels + layer3_out_channels, layer1_out_channels),
                        nn.TransformerEncoder(
                            nn.TransformerEncoderLayer(d_model=layer1_out_channels, nhead=8, dropout=trans_dropout),
                            num_layers=trans_blocks,
                        ),
                    ...
                    nn.TransformerEncoder(
                        nn.TransformerEncoderLayer(d_model=layer1_out_channels + layer4_out_channels, nhead=8, dropout=trans_dropout),
                        num_layers=trans_blocks,
                    ),
                ]
            )
            self.transformer = transformer_list
            nfc = nfc + layer1_out_channels
            self.attention = nn.Sequential(nn.Linear(nfc, 2048), nn.Tanh(), nn.Linear(2048, 1))
