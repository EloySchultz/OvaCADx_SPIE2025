...
class MILDataset(Dataset):
    ...
    def __init__(
        ...
        self.annotations = self.annotations.loc[(self.annotations[self.label] == 0) | (self.annotations[self.label] == 1)]
        self.annotations = self.annotations.loc[self.annotations["outlier"] == 0] if remove_outliers else self.annotations
...
class MILNestedKFoldModule(LightningDataModule):
    """
    ...
    def setup(self, stage: str):
        ...
        external_indices = full_dataset.annotations.index[full_dataset.annotations[self.hparams.key_external_testset] == 1].tolist()
        ...
        kf_outer = StratifiedGroupKFold(n_splits=self.hparams.num_outer_splits, random_state=self.hparams.seed, shuffle=True)
        ...
        test_indeces = external_indices + [indices[idx] for idx in test_indeces]
        ...
        if stage == "fit":
            ...
            kf_inner = StratifiedGroupKFold(n_splits=self.hparams.num_inner_splits, random_state=self.hparams.seed, shuffle=True)
    ...
    def train_dataloader(self):
        return DataLoader(dataset=self.train_dataset, batch_size=self.hparams.batch_size, num_workers=self.hparams.num_workers,
                          pin_memory=self.hparams.pin_memory, shuffle=True, collate_fn=self.hparams.collate_fn)

    def val_dataloader(self):
        return DataLoader(dataset=self.val_dataset, batch_size=self.hparams.batch_size, num_workers=self.hparams.num_workers,
